...
export default function Writing({ playerId, gameState, setGameState, send }) {
  ...
  if (gameState?.type === "next_writing" && waitingForNext) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16, paddingTop: 24 }}>
      ...
      {isBattery && serverLeftAdj && (
        <div style={{ background: "#16213e", borderRadius: 12, padding: "12px 16px", border: "1px solid #2a2a4a", textAlign: "center" }}>
          <p style={{ color: "#aaa", fontSize: 12, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6, margin: "0 0 6px" }}>Tus adjetivos</p>
      ...
      <div style={{ display: "flex", justifyContent: "center" }}>
        {targetPosition !== undefined && targetPosition !== null
          ? <Dial targetPosition={targetPosition} showTarget={true} leftAdjective={displayLeft} rightAdjective={displayRight} />
      ...
      {!waitingForNext ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {!isBattery && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div>
                ...
                <input placeholder="Ej: Frío" value={leftAdj} onChange={e => setLeftAdj(e.target.value)} style={inputStyle} autoComplete="off" />
              </div>
              <div>
                <label style={labelStyle}>➡️ Derecha</label>
                <input placeholder="Ej: Caliente" value={rightAdj} onChange={e => setRightAdj(e.target.value)} style={inputStyle} autoComplete="off" />
              </div>
            </div>
          )}
          ...
          <button
            ...
            style={{
              ...
              fontSize: 16, fontWeight: 700, cursor: canSubmit ? "pointer" : "default", marginTop: 4
            }}
          >
        ...
        <div style={{ background: "#16213e", borderRadius: 16, padding: 20, border: "1px solid #2a2a4a", textAlign: "center" }}>
          ...
          <p style={{ color: "#888", fontSize: 14, marginTop: 8 }}>Esperando a los demás jugadores...</p>
        </div>
      )}
    </div>
  )
